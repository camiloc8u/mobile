...
class _ProfileTabState extends State<ProfileTab> {
  ...
  String documento = 'No registrado'; // Campo visual para el documento
  ...
  void _sincronizarDatos() {
    if (widget.usuario != null) {
      setState(() {
        ...
        nombre = '$pNombre $sNombre $pApellido $sApellido'.replaceAll(RegExp(r'\s+'), ' ').trim();
            
        email = widget.usuario!['correo'] ?? '';
        telefono = widget.usuario!['telefono']?.toString() ?? 'No registrado'; // Aseguramos leer el teléfono
        documento = widget.usuario!['num_doc']?.toString() ?? 'No registrado'; // Leemos el documento
  ...
  Future<void> _actualizarPerfilBackend(String nuevoNombreCompleto, String nuevoEmail, String nuevoTelefono, String nuevoPlan) async {
    final int? userId = widget.usuario?['idusuario'] ?? widget.usuario?['id'];
    if (userId == null) return;

    try {
      List<String> partesNombre = nuevoNombreCompleto.trim().split(' ');
      String pNombre = partesNombre.isNotEmpty ? partesNombre[0] : '';
      String pApellido = partesNombre.length > 1 ? partesNombre.sublist(1).join(' ') : '';

      // Enviamos la data empaquetada a UsuarioService
      await UsuarioService.actualizarPerfilCliente(userId, {
        'primer_nombre': pNombre,
        'primer_apellido': pApellido,
        'correo': nuevoEmail,
        'telefono': nuevoTelefono, // El teléfono viaja aquí
  ...
  void _mostrarModalEditarPerfil() {
    ...
    _telefonoController.text = (telefono == 'No registrado') ? '' : telefono;
    _planSeleccionado = ['PREMIUM', 'BASICO', 'VIP'].contains(planActual) ? planActual : 'PREMIUM';
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 6),
                    TextField(controller: _emailController, style: const TextStyle(color: Colors.white, fontSize: 14), keyboardType: TextInputType.emailAddress, decoration: _inputDecoration()),
                    const SizedBox(height: 12),
                    
                    const Text('TELÉFONO', style: TextStyle(color: Colors.grey, fontSize: 11)),
                    const SizedBox(height: 6),
                    TextField(controller: _telefonoController, style: const TextStyle(color: Colors.white, fontSize: 14), keyboardType: TextInputType.phone, decoration: _inputDecoration()),
                    const SizedBox(height: 12),
  ...
  Widget build(BuildContext context) {
    if (widget.usuario == null) {
      return const Center(child: Text('Inicia sesión para ver tu perfil', style: TextStyle(color: Colors.grey, fontSize: 14)));
    }
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 20),
              const Text('MEMBRESÍA', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold)),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 2),
          Text(value, style: TextStyle(color: isHighlight ? const Color(0xFFAEE000) : Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
        ],
      ),
    );
  }
